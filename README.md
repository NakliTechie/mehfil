...
> **Status:** v2 + a fleet of daily-driver polish. The full Slack-benchmark feature set — channels, DMs, threads, reactions, files, huddles with screen share, Canvas collab docs, voice/screen messages, polls, checklists, scheduled send, search, ⌘K switcher, and more — plus PWA-installable offline shell and a 19-test in-browser suite. See [docs/features.md](docs/features.md) for the complete list. **Alpha stage — wire/storage breakage is acceptable.**
...
- **One HTML file** — the entire app (markup, styles, ~15,500 lines of vanilla JS) in a single ~630 KB file. No framework, no build step.
