...
- **Self-organizing mesh** — you scan **one** QR to join, and the app auto-wires direct connections to several other members through the mesh (bounded-degree, self-healing). The group stays connected even when the person who invited you — or the original host — leaves. Validated by automated real-WebRTC tests **from 4 up to 30 peers** — including under simulated latency + packet loss, and across multi-actor feature journeys (`scripts/verify-mesh-scale.mjs`, `verify-journeys.mjs`); see [docs/architecture.md](docs/architecture.md).
